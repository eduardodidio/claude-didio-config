...
## F16 — 2026-05-08
**What worked:** AC validation commands as first-class deliverables (grep/python3/bash-n per AC) made the QA pass a scripted checklist rather than judgment calls. Reading the TechLead's two IMPORTANT items up front focused effort: fix `/tmp/poc-dev.log` + hardcoded timeouts directly in QA pass. Synthetic fixture committed for T09 unblocked `F16_FAST=1` CI run without an API key.

**What to avoid:** Accepting "fixture will be generated by T08" without checking if it was actually committed. T08 requires a live API; the fixture was never in the repo. Pattern: always `ls tests/fixtures/<FXX>-*` before running integration tests — if the fixture is missing, create a synthetic one validated against the schema.

**Pattern to repeat:** For code-generation features, check generated JSX/TSX for React correctness (not just syntax). `Math.random()` keys and similar patterns are silent bugs — they won't fail a build check but degrade runtime behavior. Include a targeted grep for known antipatterns (`Math.random`, `any` type, inline styles) as part of QA checklist for codegen features.

## F24 — 2026-05-13

**What worked:** Reading the tech-lead review files first (before inspecting code) immediately surfaced the BLOCKING DIDIO_CI doc error; reading `review-20260514T001652Z.md` (the follow-up) revealed it was still unresolved, focusing QA effort precisely.
