...
## F16 — 2026-05-08
**What worked:** Schema-first design (Wave 0 manifest schema as canonical contract) aligned LLM prompt, pipeline validation, and tests automatically — changing the schema breaks all consumers immediately. `log_fn` callback injection kept codegen and finalize modules independently testable without importing the orchestrator.
...
## F26 — 2026-06-07
**What worked:** The `${DIDIO_HOME:-$HOME/.claude-didio-config}` / `${DIDIO_SECOND_BRAIN_HOME:-$HOME/didio-second-brain-claude}` portability convention (already established in `didio-config-lib.sh`) was correctly reapplied to fix three independent hardcoded-path findings (3a, 3b, plus the settings template gaps) — one decision, reused everywhere, no new pattern invented.
