...
if [[ -d "$DASHBOARD_DIST" && -f "$DASHBOARD_DIST/index.html" ]]; then
  ...
  # Expose logs/agents/meta-metrics.json inside the served dir (relative fetch)
  ln -sfn "$LOG_DIR/meta-metrics.json" "$DASHBOARD_DIST/meta-metrics.json"
  ...
  # Seed meta-metrics.json so the first fetch doesn't 404
  [[ -f "$LOG_DIR/meta-metrics.json" ]] || \
    echo '{"generated_at":"","gandalf":{},"saruman":{},"loop_health":{}}' \
    > "$LOG_DIR/meta-metrics.json"
